...
import com.sp.location.NetworkEquipmentDetail;
...
public class NetworkEquipmentReportForm extends Panel {
	...
	public NetworkEquipmentReportForm(String id) {
		super(id);
		setDefaultModel(new CompoundPropertyModel<NetworkEquipmentReportForm>(this));
		...
		columns.add(new PropertyColumn(new Model("Equipment Name"), "equipmentdesc"));
		columns.add(new PropertyColumn(new Model("Make"), "make"));
		columns.add(new PropertyColumn(new Model("Model"), "model"));
		columns.add(new PropertyColumn(new Model("Serial No"), "serialnumber"));
		columns.add(new PropertyColumn(new Model("AMC/Warranty"), "amc"));
		columns.add(new PropertyColumn(new Model("Installation Date"), "installdate"));
	...
	public class NetworkLocationDataProvider extends SortableDataProvider implements Serializable {
		...
		public Iterator iterator(long first, long count) {
			// TODO Auto-generated method stub
			Collections.sort(nrsdlist.getList(), new Comparator<NetworkEquipmentDetail>() {
	...
	public List<NetworkEquipmentDetail> getAllNetworkLocations() {
		final List<NetworkEquipmentDetail> list = new ArrayList<NetworkEquipmentDetail>();
		final String query = "{call sp_circuit_network_equipment_report(?,?,?,?,?,?,?)}";
		Connection con = null;
		...
		try {
			...
			while (rs.next()) {
				list.add(new NetworkEquipmentDetail(rs.getString(1),rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8)));
			}
