...
public class NoNetworkReportForm extends Panel {
	private static final Logger log = Logger.getLogger(NoNetworkReportForm.class);
	...
	public NoNetworkReportForm(String id) {
		super(id);
		setDefaultModel(new CompoundPropertyModel<NoNetworkReportForm>(this));
		...
		columns.add(new PropertyColumn(new Model("Service Type"), "servicetype"));
			...
			while (rs.next()) {
				if (rs.getInt(13) == 5) {
					list.add(new NetworkLocationDetail(rs.getString(1), rs.getString(2), rs.getInt(3), rs.getString(4),
							...
							rs.getString(10), rs.getString(11), rs.getString(12), rs.getString(16)));
				}
			}
