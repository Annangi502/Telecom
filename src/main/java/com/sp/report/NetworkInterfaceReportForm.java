...
import com.sp.location.NetworkInterfaceDetail;
import com.sp.location.NetworkInterfaceDetail;
...
public class NetworkInterfaceReportForm extends Panel {
	private static final Logger log = Logger.getLogger(NetworkInterfaceReportForm.class);
	...
	public NetworkInterfaceReportForm(String id) {
		super(id);
		setDefaultModel(new CompoundPropertyModel<NetworkInterfaceReportForm>(this));
		StatelessForm<Form> form = new StatelessForm("form");
		nrsdlist.setList(getAllNetworkLocations());
		...
		columns.add(new PropertyColumn(new Model("Project Type"), "projecttype"));
		columns.add(new PropertyColumn(new Model("Vendor Name"), "vendorname"));
		columns.add(new PropertyColumn(new Model("Interface"), "ntinterface"));
		columns.add(new PropertyColumn(new Model("Lan Ip Address"), "ipaddress"));
	...
	public class NetworkLocationDataProvider extends SortableDataProvider implements Serializable {
		...
		public Iterator iterator(long first, long count) {
			// TODO Auto-generated method stub
			Collections.sort(nrsdlist.getList(), new Comparator<NetworkInterfaceDetail>() {

				@Override
				public int compare(NetworkInterfaceDetail arg0, NetworkInterfaceDetail arg1) {
					// TODO Auto-generated method stub
					int dir = getSort().isAscending() ? 1 : -1;
					return dir * (arg0.getLocationname().compareTo(arg1.getLocationname()));
				}
			});
	...
	public List<NetworkInterfaceDetail> getAllNetworkLocations() {
		final List<NetworkInterfaceDetail> list = new ArrayList<NetworkInterfaceDetail>();
		final String query = "{call sp_circuit_network_interface_report(?,?,?,?,?,?,?)}";
		Connection con = null;
		...
		try {
			...
			while (rs.next()) {
				list.add(new NetworkInterfaceDetail(rs.getString(1),rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
			}
