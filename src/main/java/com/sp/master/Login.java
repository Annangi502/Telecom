	...
	private boolean doLogin() {
		...
		try {
			...
			while (rs.next()) {
				...
				((PortSession) getSession()).setWing(rs.getString(7));
				((PortSession) getSession()).setCircleid(rs.getInt(8));
			...
			log.error("SQL Exception in doLogin() method {" + e.getMessage() +", Error Code :"+e.getErrorCode()+ "}");
			e.printStackTrace();
			return false;
		} finally {
		}
