...
public class DataTablePanel extends Panel {
	...
	public DataTablePanel(String id,List<NetworkLocationDetail> locationslist) {
		super(id);
		setDefaultModel(new CompoundPropertyModel<DataTablePanel>(this));
	    StatelessForm<Form> form = new StatelessForm("form");
		ntlclist.setList(locationslist);
	...
	/*	columns.add(new GClickablePropertyColumn(new Model("Circuit ID"), "spciruitcode") {
	  ...
	  /*  columns.add(new PropertyColumn(new Model("Location Name"), "locationname"));*/
	    columns.add(new GClickablePropertyColumn(new Model("Network Status"), "status") {
			public void populateItem(Item item, String componentId, IModel rowModel) {
				item.add(new ColumnClickPanelNetworkLocationDetail(componentId, rowModel,
						new PropertyModel(rowModel, getProperty())));
			}
		});
	    columns.add(new PropertyColumn(new Model("Lan IP"), "lanip"));
	...
	private List<SubDivision> loadSubDivisions(final int circleid, final int divisionid,int eroid) {
