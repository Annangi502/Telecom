...
public class EditInterruptionsDetailsForm extends Panel {
	private static final Logger log = Logger.getLogger(EditInterruptionsDetailsForm.class);
	...
	private  Date date2 ;
	private  Date date3 ;
	private InterruptionDetails nud;
	...
	*/
	...
	public EditInterruptionsDetailsForm(String id,final IModel<InterruptionDetails> nldmodel) {
		super(id);
		// TODO Auto-generated constructor stub
		setDefaultModel(new CompoundPropertyModel<EditInterruptionsDetailsForm>(this));
		StatelessForm<Form> form = new StatelessForm<Form>("editinterruptionform");
		...
		try {
			nud = nldmodel.getObject();			
			circuitdesc = nud.getCircuitid() ;			
			locationname = nud.getLocationname() ;		
			vendor = nud.getVendorname();			
			linkdowndate = nud.getLinkdowntime();			
		    linkupdate = nud.getLinkuptime();	
		    calllog = nud.getCalllog();	
		    
		    SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
		    date2 = sdf.parse(linkdowndate);
		    long millis = date2.getTime();		    
		    date2 = new Date(millis);
		    
		    date3 = sdf.parse(linkupdate);
		    ...
		    //System.out.println(" linkdowndate "+linkdowndate);
		    //System.out.println(" linkupdate "+linkupdate);
		...
		final DropDownChoice<CircuitId> circuitidselect = new DropDownChoice<CircuitId>("circuitid", circuitidlist,
			...
			protected void onSelectionChanged(CircuitId newSelection) {
				...
				//System.out.println(" locationname 111 "+locationname);
	...
	private boolean editInterrutionDetails() {
      ...
      /* long diff = date3.getTime() - date2.getTime();
       ...
       long minutes = total.minusHours(hours).toMinutes();
		...
		final String query = "{call sp_circuit_edit_interrutions_details(?,?,?,?,?,?,?,?)}";
		
		Connection con = null;
		CallableStatement stmt = null;
		ResultSet rs = null;
		//System.out.println(" circuitid "+circuitdesc);
		...
		try {
			...
			while (rs.next()) {
				log.info("Edit Interruptions Detail Added Successfully With Circuit ID is :" + rs.getString(1));
			}
		} catch (SQLException e) {
			log.error("SQL Exception in sp_circuit_edit_interrutions_details() method {" + e.getMessage() + "}");
			e.printStackTrace();
			return false;
		} finally {
			try {
				...
				log.error("SQL Exception in sp_circuit_edit_interrutions_details() method {" + e2.getMessage() + "}");
				e2.printStackTrace();
			}
		}
		return true;
	...
	private List<CircuitId> loadCircuitIds() {
		...
		try {
			...
			while (rs.next()) {
				...
				CircuitId cid = new CircuitId(rs.getString(1), rs.getString(2), rs.getString(3));
				if (circuitdesc.trim().equals(rs.getString(1).trim())) {
					circuitid = cid;
				}
